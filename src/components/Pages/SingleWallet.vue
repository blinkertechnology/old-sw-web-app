        ...
        <kaiui-tab-item name="Wallet Details" selected>
            ...
            <div v-else>
                <ul class="demo" nav-selectable="true">
                    <li>Token : <b>{{ singlewallet ? singlewallet.secretType.charAt(0).toUpperCase() + singlewallet.secretType.slice(1).toLowerCase() : "" }}</b></li>
...
export default {
    ...
    methods: {
        softkeysLeftprevious (){
            this.loading = true
            this.fetchWalletIds()
            var num = this.items.findIndex(element => element.id === this.$route.params.id)
        ...
        softkeysRightBack () {
            this.$router.push({ name: "transactionslist", params: { secretType:this.singlewallet.address }, query: { walletId: this.$route.params.id }})
        },
        fetchWalletIds () {
            this.loading = true
